...
<?php if ($resultats != null) : ?>
        ...
        <div class="row">
            ...
            <div class="col-md-6 my-auto">
                ...
                <?php if (empty($moyenne["moyenne"])): ?>
                    <p class="mt-5">Aucun commentaire</p>
                <?php elseif (count($moyenne) == 1): ?>
                    <p class="mt-5"><?= round($moyenne["moyenne"], 2) . " " . generateStars($moyenne["moyenne"]) . " " . count($moyenne) . " commentaire" ?></p>
                <?php else : ?>
                    <p class="mt-5"><?= round($moyenne["moyenne"], 2) . " " . generateStars($moyenne["moyenne"]) . " " . count($moyenne) . " commentaires" ?></p>
                <?php endif; ?>
                <p>Date de sortie : <?= date("d/m/Y", strtotime($resultat["date_sortie"])) ?> </p>
    ...
    <div class="container border-3 my-5">
        <div class="border-bottom mb-5 row">
            ...
            <?php else : ?>
                <button type="button" class="col btn-sm btn btn-info my-auto text-white" disabled
                        data-bs-toggle="button">Ajouter un
                    commentaire
                </button>
            <?php endif; ?>
        </div>
        ...
        <?php else : ?>
            <?php foreach ($commentaires as $commentaire): ?>
                <ul class="container list-group border p-3">
                    <h2>
                    ...
                    <p>
                        ...
                        à <?= date("H:i", strtotime($commentaire["heure_commentaire"])) ?>
                    </p>
                </ul>
            <?php endforeach; ?>
        <?php endif; ?>
